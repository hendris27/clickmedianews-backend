...
exports.getSavePost = async function(request, response){
    try {
        ...
        const {rows: results, pageInfo} = await savePosteModel.findAll(request.params, id);
...
exports.deleteSavePost = async function(request, response){
    try {
        const data = await savePosteModel.findOne(request.body.id, request.user);
        if(!data){
            throw Error("article not found");
        }
        const dltArticle = await savePosteModel.destroy(request.body.id, request.user);
        if(dltArticle){
            return response.json({
                success: true,
